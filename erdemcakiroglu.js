...
if(gif > 0 && pp > 0){
  ...
  client.channels.cache.get(client.gif.log).send(new Discord.MessageEmbed().setColor('BLUE').setAuthor(`🌙 ${client.gif.sunucu} 🌙`).setDescription(`${emojis} **Gönderen:** ${msg.author} \`(${msg.author.id})\`\n**Gönderilen Kanal:** <#${msg.channel.id}> \`(${msg.channel.id})\`\n**Gönderilen:** \`${mesaj}\` Gönderilmiştir!\n**Toplam Gönderilen:** \`${db.fetch(`sayı.${msg.author.id}`)||0} PP/Gif\` Gönderilmiş!`))
}
})
