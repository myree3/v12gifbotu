...
exports.run = async(client, message, args) => {
  ...
  if(!args[1]) return message.channel.send(new Discord.MessageEmbed().setColor('BLUE').setDescription(`Bir Kategori Gir!`));
   if(!['gif','pp'].includes(args[1])) return message.channel.send(new Discord.MessageEmbed().setColor('BLUE').setDescription(`Kategoriyi Doğru Gir!`));
  ...
  if(args[2].startsWith('-')) return message.channel.send(new Discord.MessageEmbed().setColor('BLUE').setDescription(`Ekleme Yaparken Dikkat Et!`));
